...
class Quadrotor():
	def __init__(self):
		...
		# TUNE
		self.Kp = 40 # using kp and kd in 'traj_evaluate' function to calculate desired theta and phi values 
		self.Kd = 3#0.1
	...
	def smc_control(self, xyz, xyz_dot, rpy, rpy_dot):
		...
		if self.iter%100 == 0:
			print("s1,zd_dot,x5,cos(x2),cos(x3): ",self.s1,zd_dot,x5,cos(x2),cos(x3))

		u1 = self.m*(self.g + zd_ddot - self.lambda1*(x5 - zd_dot) - self.k1*self.sat(self.s1))/(cos(x2)*cos(x3))


		#u1 =(-self.k1*(x1-zd) - 0.3*self.k1*(x5-zd_dot) + self.g)*self.m
		...
		PHI_d =  math.asin(-Fy/den) # phi desired --> not using the variable name 'phi' it as a tuning parameter name
		psi_d = 0
		...
		if (res) and self.iter%100 ==0:
			#print(f"{'\033[93m'} Warning: MAX VALUE REACHED " )
			print(f"{WARNING}Warning: MAX VALUE REACHED{ENDC}")
